	...
	module.exports = function(target) {
		var RPCRequest = new Class({
			...
			, init: function(options) {
				...
				if (options) {
					if (options.language) {
						this._request.setLanguages([options.language]);
						this.language = options.language;
					}
					if (options.languages) this._request.setLanguages(options.languages);
			...
			, _storeFilter: function(filter) {
				var regResult = /([a-z0-9\._]+)(\=([a-z0-9_]+)\(([^\)]+)\)|([=<>\!]+)"?([^"]+)"?)$/gi.exec(filter.trim())
					, key
					, operator
					, value;
				...
				if (regResult) {
					if (regResult[4]) {
						...
						value 		= function() {
							return {
								  name: regResult[3].trim()
								, parameters: regResult[4].split(/\s?,\s?/gi)
							}
						};
					}
					else {
						key  		= regResult[1].trim().split('.');
						operator 	= regResult[5];
						value 		= regResult[6];
					}					
